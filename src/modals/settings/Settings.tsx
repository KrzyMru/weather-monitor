    ...
    return (
        ...
        >
            <div className="flex items-center justify-center">
                ...
                <p className="text-2xl font-bold line-clamp-1 transition-[color] duration-350 dark:text-white">{t('settings.header')}</p>
            </div>
            <div className="space-y-4 px-5 pt-4 pb-2 sm:px-20">
                ...
                >
                    <div className="flex items-center border-b-1 p-2 transition-[border-color] duration-350 border-gray-300 dark:border-gray-500">
                        ...
                        <p className="text-xl font-semibold line-clamp-1 font-mono transition-[color] duration-350 text-gray-300 dark:text-white">{t('settings.darkModeHeader')}</p>
                    </div>
                    <div className="p-2">
                        <p className="text-left text-base transition-[color] duration-350 text-gray-300 dark:text-white">{t('settings.darkModeText')}</p>
                    </div>
                </button>
                ...
                >
                    <div className={`flex items-center border-b-1 p-2 transition-[border-color] duration-350 ${chooseLocationOnClick ? 'border-white dark:border-gray-500' : 'border-gray-300 dark:border-gray-600'}`}>
                        ...
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24" 
                            className={`size-[28px] mr-2 shrink-0 transition-[fill] duration-350 ${chooseLocationOnClick ? 'fill-white' : 'fill-gray-300 dark:fill-gray-600'}`}
                        >
                        ...
                        <p className={`text-xl font-semibold line-clamp-1 font-mono transition-[color] duration-350 ${chooseLocationOnClick ? 'text-white' : 'text-gray-300 dark:text-gray-600'}`}>{t('settings.forecastOnClickHeader')}</p>
                    </div>
                    <div className="p-2">
                        <p className={`text-left text-base transition-[color] duration-350 ${chooseLocationOnClick ? 'text-white' : 'text-gray-300 dark:text-gray-600'}`}>{t('settings.forecastOnClickText')}</p>
                    </div>
                </button>

                {/* Language */}
                <button 
                    className={`rounded-lg shadow-md border-1 transition-[background-color,border-color,outline-width] duration-[350ms,350ms,0ms] bg-rose-300 border-gray-400 dark:bg-gray-600 dark:hover:bg-gray-700 dark:active:bg-gray-900 hover:bg-rose-400 active:bg-rose-500 hover:cursor-pointer focus-visible:outline-2 focus-visible:outline-slate-600 dark:focus-visible:outline-slate-300`}
                    onClick={() => {
                    ...
                    title={t('settings.forecastOnClickButtonTitle')}
                >
                    <div className="flex items-center border-b-1 p-2 border-white">
                        <p className="text-2xl font-bold line-clamp-1 font-mono text-white uppercase mr-2 bg-sky-300 shadow-sm px-1 rounded-full [transition:background-color_350ms] dark:bg-sky-600 ">{i18n.language}</p>
