...
const WeatherForecast = (props: WeatherForecastProps) => {
    ...
    // Center view on chosen time if possible
    React.useEffect(() => {
        selectedTimeRef.current?.scrollIntoView({ behavior: "smooth", inline: "center", block: "center" });
    }, [selectedTime])
            ...
            <div className="flex flex-col justify-center">
                ...
                <div className="mb-8">
                    <div className="flex justify-center items-center">
                        <SelectedTimeWeatherCodeIcon className="size-[108px] shrink-0" />
                        <div className="items-center w-[160px]">
                            <div className="flex">
                                <p className="font-bold text-5xl dark:text-white">{selectedTimeData.temperature_2m}</p>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex">
                                    <p className="font-semibold text-sm dark:text-white">{selectedTimeData.apparent_temperature}</p>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center">
                                    <p className="font-semibold text-sm dark:text-white">{selectedTimeData.precipitation_probability}</p>
                                    <p className="font-semibold text-sm dark:text-white">{precipitation_probability_unit}</p>
                                </div>
                            </div>
                            <p className="text-sm text-gray-600 dark:text-gray-300">{SelectedTimeWeatherCodeName}</p>
                        </div>
                    </div>
                    <div className="flex justify-center items-end">
                        ...
                        >
                            <InfoOutline className="size-[16px] bg-white fill-white rounded-full shrink-0 dark:bg-gray-600 group-hover:fill-gray-100 group-active:fill-gray-200"/>
                        </button> 
                    </div>
                    <div className={`flex flex-col items-center overflow-hidden transition-[max-height] duration-500 ease-in-out ${openGeolocationInfo ? 'max-h-100' : 'max-h-0'}`}>
                        {geolocationLabels.map((label) => (
                            <p className="text-base font-mono first-letter:uppercase dark:text-white">{label}</p>
                        ))}
                        <p className="text-sm font-mono first-letter:uppercase dark:text-white">{'Lat: ' + geocodingData.lat}</p>
                        <p className="text-sm font-mono first-letter:uppercase dark:text-white">{'Lon: ' + geocodingData.lon}</p>
                    </div>
                    <div className="flex justify-around">
                        <p className="font-mono first-letter:uppercase text-base indent-1 dark:text-white">{selectedTime.format('dddd, D MMMM HH:mm')}</p>     
                    </div>
                </div>

                {/* Hour list */}
                <ul className="flex flex-nowrap overflow-x-scroll space-x-1 p-2 bg-gray-50 rounded-xl dark:bg-gray-700 focus-visible:outline-3 focus-visible:outline-slate-500">
                    {SelectedTimeDayWeatherDataArray.map((data, index) => {
                        ...
                        <li className="inline-block" key={index}>
                            <button 
                                className="flex flex-col items-center p-2 mb-2 bg-white rounded-lg shadow-md outline-none dark:bg-gray-600 dark:hover:bg-slate-700 dark:active:bg-slate-800 dark:disabled:bg-slate-800 hover:cursor-pointer hover:bg-sky-50 active:bg-sky-100 disabled:bg-sky-100 disabled:cursor-default focus-visible:inset-ring-3 focus-visible:inset-ring-slate-500"
                ...
                {/* Day list */}
                <ul className="space-y-1 mt-5 p-4 bg-gray-50 rounded-xl dark:bg-gray-700">
                    {weatherDataDailyArray.map((day, index) => {
                        ...
                        return (
                            <li key={index}>
                                <button 
                                    className="w-full flex justify-between items-center p-2 mb-2 bg-white shadow-md outline-none dark:bg-gray-600 dark:hover:bg-slate-700 dark:active:bg-slate-800 dark:disabled:bg-slate-800 hover:cursor-pointer hover:bg-sky-50 active:bg-sky-100 disabled:bg-sky-100 disabled:cursor-default focus-visible:inset-ring-3 focus-visible:inset-ring-slate-500"
                                ...
                                >
                                    <p className="text-left first-letter:uppercase font-semibold text-base line-clamp-1 w-[150px] dark:text-white">{day.time.format('dddd')}</p>
                                    <div className="flex justify-between w-[100px]">
                                        <div className="flex justify-center items-center">
                                            <PrecipitationProbabilityIcon className="size-[28px] shrink-0"/>
                                            <p className="text-gray-600 text-xs dark:text-gray-300">{day.precipitation_probability_max}</p>
                                            <p className="text-gray-600 text-xs dark:text-gray-300">{precipitation_probability_unit_max}</p>
                                        </div>
                                        <day.weather_icon className="size-[32px] bg-white rounded-full shrink-0 dark:bg-gray-500"/>
                                    </div>
                                    <div className="flex justify-end w-[150px]">
                                        <div className="flex justify-center">
                                            <p className="font-semibold text-sm dark:text-white">{day.temperature_2m_max}</p>
                                            <p className="font-semibold text-xs dark:text-white">{temperature_2m_unit_max}</p>
                                        </div>
                                        <p className="font-semibold text-sm indent-1 text-gray-600 dark:text-gray-300">/</p>
                                        <div className="flex justify-center">
                                            <p className="font-semibold text-sm indent-1 dark:text-white">{day.temperature_2m_min}</p>
                                            <p className="font-semibold text-xs dark:text-white">{temperature_2m_unit_min}</p>
                                        </div> 
                                    </div>
                                </button>
                            </li>
