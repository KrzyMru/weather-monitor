...
const MainBanner = (props: MainBannerProps) => {
    const { 
        ...
        geolocationData, onLocationFavouriteClick
    } = { ...props };
    ...
    const FavouriteIcon = geolocationData.is_favourite ? HeartBroken : Heart;
    const weatherName = t('weatherForecast.weatherNames.'+weatherData.weather_code_data.nameString);
    const geolocationLabels = geolocationData.display_name.split(',').slice(1);
        ...
        <div className="mb-8">
            ...
            <div className="flex justify-center items-center">
                <button 
                    ...
                    title={geolocationData.is_favourite ? t('unfavouriteButtonTitle') : t('favouriteButtonTitle')}
                    type="button"
                    onClick={() => onLocationFavouriteClick(geolocationData)}
                    disabled={geolocationData.osm_id === -1} // Can't favourite error location
                >
                    <FavouriteIcon className={`size-[22px] shrink-0 ${geolocationData.osm_id === -1 ? 'dark:fill-slate-600 fill-slate-200' : 'fill-rose-400'}`} />
                </button>
                <p className="text-center font-semibold text-lg dark:text-white">{geolocationData.name}</p> 
                <button 
            ...
            <div className={`flex flex-col items-center overflow-hidden transition-[max-height] duration-350 ease-in-out ${openGeolocationInfo ? 'max-h-100' : 'max-h-0'}`}>
                ...
                <p className="text-gray-600 text-sm font-mono first-letter:uppercase dark:text-gray-300">{'Lat: ' + geolocationData.lat}</p>
                <p className="text-gray-600 text-sm font-mono first-letter:uppercase dark:text-gray-300">{'Lon: ' + geolocationData.lon}</p>
            </div>
